...
| Cordis 插件 ×3 | roycode-hooks v2（**可编程事件引擎**：session 事件→shell 命令，4 个运行时管理工具 hooks_rule_add/confirm/remove/list + JSON 持久化 + 会话事件审计）、roycode-teams v0.3（**11 个工具**：成员读游标 + unread + limit、team_archive 幂等归档（默认保留 200 条）、内存上限 50 + memoryHistory、team_memory_clear）、dsh-schedule（原生定时提醒：schedule_create/list/delete，scheduled-prompts 技能的引擎） | 需重启 dsh web |
