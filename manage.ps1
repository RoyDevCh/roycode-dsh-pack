...
$PACK_IDS = @('mcp-lsp', 'mcp-secret-scan', 'mcp-browser', 'mcp-notebooks', 'mcp-voice', 'roycode-hooks', 'roycode-teams', 'schedule', 'roycode-triggers', 'roycode-inventory', 'roycode-voice')
...
function Show-Status {
  ...
  $pluginDirs = @('roycode-hooks','roycode-teams','roycode-triggers','roycode-inventory','roycode-voice' | Where-Object { Test-Path (Join-Path $PluginRoot $_) })
  Write-Host ''
  ...
  Write-Host ('cordis plugin dirs: {0}/5' -f $pluginDirs.Count) -ForegroundColor Cyan
  Write-Host ''
